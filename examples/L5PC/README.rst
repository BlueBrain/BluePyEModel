...
* ``morph_modifiers`` specifies morph modifiers. Use a string for a named modifier or a list with [file path, function name, optional "hoc_string"]. If not specified, a default modifier (replace_axon_with_taper) is applied which will replace the axon with a tapered axon initial segment (note that this requires a morpholgy with 3 axon sections). Predefined modifiers can be found in the `modifiers.py <../../bluepyemodel/evaluation/modifiers.py>`_ e.g., ["replace_axon_olfactory_bulb"]. If the morphology does not have an axon, set it to ["bluepyopt_replace_axon"]. Set to [] to skip modifiers.
