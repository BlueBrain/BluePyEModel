...
* ``morph_modifiers`` specifies morph modifiers. Use a string for a named modifier or a list with [file path, function name, optional "hoc_string"]. If not specified, a default modifier (replace_axon_with_taper) is applied which will replace the axon with a tappered axon initial segment. Set to [] to skip modifiers.
