...
def ipyparallel_map_function(os_env_profile="IPYTHON_PROFILE", profile=None):
    ...
    Args:
        os_env_profile (str): The name of the environment variable that holds the profile name for ipyparallel. If this environment variable
        exists and contains a valid profile name, it will be used to create an ipyparallel Client. Defaults to "IPYTHON_PROFILE"
    ...
    if profile:
        ...
        logger.warning("Environment variable 'os_env_profile' not set or invalid; falling back to the default map function.")
        mapper = map
