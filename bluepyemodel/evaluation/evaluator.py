...
def define_main_protocol(
    ...
    if any(isinstance(p, BPEM_DynamicStepProtocol) for p in dynamic_protocols.values()):
        for suffix in ["_noburst", "_burst"]:
            ...
            step_protocol_name = f"TRNSearchCurrentStep{suffix}"
            holding_current_name = f"TRNSearchHolding_current{suffix}"
            step_current_name = f"TRNSearchCurrentStep_current{suffix}"

            pre_protocols[hold_protocol_name] = define_current_for_voltage_protocol(
                efeatures,
                protocol_name=hold_protocol_name,
                target_current_name=holding_current_name,
                strict_bounds=strict_holding_bounds,
            )
            pre_protocols[step_protocol_name] = define_current_for_voltage_protocol(
                efeatures,
                protocol_name=step_protocol_name,
                target_current_name=step_current_name,
                upper_bound=0.1,
                lower_bound=-0.4,
                strict_bounds=strict_holding_bounds,
            )
