...
from bluepyemodel.tools.utils import get_checkpoint_path, read_checkpoint
...
class DataAccessPoint:
    ...
    def __str__(self):
        ...
        str_ += f"  Has a fitness calculator configuration: {self.has_fitness_calculator_configuration()}\n"
        ...
        if emodels:
            ...
            str_ += f"  Number of validated emodel: {n_emodels_validated} ({n_emodels_validated / n_emodels} %)\n"
            str_ += f"  Best emodel: seed {emodels[idx_best].seed} with fitness {emodels[idx_best].fitness}\n\n"
        else:
            str_ += f"  No emodels\n\n"
