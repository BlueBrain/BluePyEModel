...
class NexusForgeAccessPoint:
            ...
            if isinstance(resource.distribution, list):
                for dist in resource.distribution:
                    if hasattr(dist, "name"):
                        filenames.append(pathlib.Path(download_directory) / dist.name)
                    else:
                        raise AttributeError("The resource does not have a file name.")
            else:
    ...
    def object_to_nexus(self, object_, metadata_dict, metadata_str, replace=True, seed=None):
        ...
        path_json = str(CLASS_TO_RESOURCE_NAME[object_.__class__.__name__]) + "__"
        path_json += metadata_str + ".json"
        path_json = str((pathlib.Path("./nexus_temp") / path_json).resolve())

        distributions = [path_json]
        if "nexus_distributions" in json_payload:
            distributions += payload.pop("nexus_distributions")
    ...
    def resource_to_object(self, type_, resource, metadata):
        ...
        json_path = next((fp for fp in file_paths if ".json" in fp), None)
